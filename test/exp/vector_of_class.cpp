...
struct oelement_t : observable::exp::class_
{
    observable::exp::value<std::string> s;
    oelement_t()
    {
...
using ovector_t = observable::exp::vector<oelement_t>;
...
int main()
{
...

    {
        ...
        oelement.on_change([&element_on_change]()
                           { element_on_change = true; });
        auto& os = oelement.s;    
        os.on_change([&s_on_change]()
                     { s_on_change = true; });
        {
            auto& oelement = ovec.front();
            oelement.on_change([&element2_on_change]()
                               { element2_on_change = true; });
            auto& os2 = oelement.s; 
            os2.on_change([&s2_on_change]()
                          { s2_on_change = true; });
        }
        oelement.s = "GHI";
